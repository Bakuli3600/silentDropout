...
def calculate_engineered_features(row):
    """Calculates all dynamic features including the new ratio signal."""
...
def run_comprehensive_tests():
    ...
    with open(RESULTS_FILE, 'w') as f:
        f.write("--- SILENT DROPOUT DETECTION SYSTEM: RESEARCH-FIXED RESULTS ---\n")
        f.write(f"Date: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
        f.write("Model: LightGBM (Gradient Boosting, Optimized Features, threshold=0.35)\n")
        f.write("-" * 50 + "\n\n")
    ...
    edge_cases = [
        ...
        {"name": "Just Scraping By (Borderline)", "attendance_rate": 50, "assignment_submission_rate": 50, "lms_login_frequency": 2, "avg_session_time": 20, "grades": 45, "expected": 1},
        ...
        {"name": "High Att, 0 Submissions (CRITICAL FIX)", "attendance_rate": 100, "assignment_submission_rate": 0, "lms_login_frequency": 5, "avg_session_time": 30, "grades": 40, "expected": 1}
    ]
    ...
    with open(RESULTS_FILE, 'a') as f:
        for i, case in enumerate(edge_cases, 1):
            ...
            f.write(f"  Input: {case['attendance_rate']}% Att, {case['assignment_submission_rate']}% Sub, Ratio: {ratio:.1f}\n")
